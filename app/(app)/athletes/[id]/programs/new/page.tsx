...
export default function NewAthleteProgramPage() {
  ...
  const handleCopyTemplate = async (template: Program) => {
    ...
    setSaving(true);
    try {
      const ref = await copyProgramToAthlete(user.uid, athleteId, template);
      if (makeActive) await setActiveAthleteProgram(user.uid, athleteId, ref.id);
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    setError("");
    try {
      const ref = await createAthleteProgram(user.uid, athleteId, {
        name: name.trim(),
        sport: sport.trim(),
        cycles,
        status: "active",
          ...
          {templates.length === 0 ? (
            ...
            <>
              <p className="text-xs text-slate-500 mb-1">
                Scegli un template dalla tua libreria — verrà creata una copia personalizzata per {athlete?.name}.
              </p>
              {templates.map((t) => (
                <button
                  key={t.id}
