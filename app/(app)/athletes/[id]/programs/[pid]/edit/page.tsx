...
import {
  getAthleteProgram,
  updateAthleteProgram,
  deleteAthleteProgram,
  setActiveAthleteProgram,
  getAthlete,
} from "@/lib/firestore";
import type { Cycle, AthleteProgram, Athlete } from "@/types";
...
export default function EditAthleteProgramPage() {
  ...
  const [_program, setProgram] = useState<AthleteProgram | null>(null);
  ...
  const [status, setStatus] = useState<AthleteProgram["status"]>("active");
  ...
  useEffect(() => {
    if (!user) return;
    Promise.all([
      getAthlete(user.uid, athleteId),
      getAthleteProgram(user.uid, athleteId, programId),
    ]).then(([a, p]) => {
      setAthlete(a);
      setProgram(p);
      if (p) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError("");
    try {
      await updateAthleteProgram(user.uid, athleteId, programId, {
        name: name.trim(),
        sport: sport.trim(),
        cycles,
        status,
      ...
      if (isActive) {
        await setActiveAthleteProgram(user.uid, athleteId, programId);
      }
  ...
  const handleDelete = async () => {
    ...
    setSaving(true);
    try {
      await deleteAthleteProgram(user.uid, athleteId, programId);
      router.push(`/athletes/${athleteId}`);
    } catch {
    ...
    <div className="px-4 pt-6 pb-8">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div className="flex-1">
          ...
          {athlete && <p className="text-xs text-slate-400">per {athlete.name}</p>}
        </div>
        <button
          type="button"
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="bg-slate-800 rounded-2xl p-4 border border-slate-700 space-y-3">
          ...
          <div>
            ...
            <select
              value={status}
              onChange={(e) => setStatus(e.target.value as AthleteProgram["status"])}
              className={inputCls}
            >
          ...
          <label className="flex items-center gap-3 cursor-pointer">
            ...
            />
            <span className="text-sm text-slate-300">Programma corrente</span>
          </label>
        </div>
